...
# define and create folder for results of ar process
ar_dir = project_root / "data" / "ar-process"
ar_dir.mkdir(parents=True, exist_ok=True)



# load data
path_monthly = project_root / "data" / "macro-vars-monthly.csv"
path_daily = project_root / "data" / "macro-vars-daily.csv"
path_weekly = project_root / "data" / "macro-vars-weekly.csv"
path_quarterly = project_root / "data" / "macro-vars-quarterly.csv"
...
#------------------------------------------------------------------
# FIXED AR(1) SPECIFICATION
#------------------------------------------------------------------
# We skip order-search validation because this benchmark uses fixed ARIMA(1,0,0).
...
for freq_name in dataframes:
    ...
    cv_naive_results = {col: {h: [] for h in horizons} for col in df_t.columns}
    ...
    for fold_i, ((train, test), (train_orig, test_orig)) in enumerate(zip(folds_transformed, folds_original)):
        ...
        for col in df_t.columns:
            for h in horizons:
                ...
                # Naive persistence baseline in original levels: y_hat(t+k) = y_train_last
                naive_last = train_orig[col].dropna().iloc[-1]
                naive_forecast = pd.Series(naive_last, index=actual_clean.index)
                naive_metrics = compute_metrics(actual_clean, naive_forecast)
                cv_naive_results[col][h].append(naive_metrics)

    all_cv_results[freq_name]   = cv_results
    all_cv_forecasts[freq_name] = cv_forecasts
    all_cv_naive_results[freq_name] = cv_naive_results
...
for freq_name in dataframes:
    ...
    naive_rows = []
    cv_res_naive = all_cv_naive_results[freq_name]
    for col, h_dict in cv_res_naive.items():
        for h, fold_metrics in h_dict.items():
            if h not in horizons or len(fold_metrics) == 0:
                continue
            mean_rmse_naive = np.mean([m["RMSE"] for m in fold_metrics])
            naive_rows.append({
                "variable":  col,
                "horizon":   h,
                "mean_RMSE": mean_rmse_naive,
                "mean_MAE":  np.mean([m["MAE"]  for m in fold_metrics]),
                "mean_MAPE": np.mean([m["MAPE"] for m in fold_metrics]),
                "NRMSE":     mean_rmse_naive / levels[col],
            })

    naive_df = pd.DataFrame(naive_rows)
    all_naive_results_df[freq_name] = naive_df

    comparison_df = results_df.merge(
        naive_df,
        on=["variable", "horizon"],
        suffixes=("_ar1", "_naive")
    )
    comparison_df["rmse_improvement_pct"] = np.where(
        comparison_df["mean_RMSE_naive"] > 0,
        (
            (comparison_df["mean_RMSE_naive"] - comparison_df["mean_RMSE_ar1"])
            / comparison_df["mean_RMSE_naive"]
        ) * 100,
        np.nan,
    )
    all_comparison_df[freq_name] = comparison_df
    ...
    print("\n  rmse_improvement_pct (AR1 vs naive):")
    print(
        comparison_df
        .pivot(index="variable", columns="horizon", values="rmse_improvement_pct")
        .round(2)
    )
        
        

with pd.ExcelWriter(ar_dir / "cv_results.xlsx", engine="openpyxl") as writer:
    for freq_name, results_df in all_results_df.items():
        results_df.to_excel(writer, sheet_name=freq_name, index=False)
        all_naive_results_df[freq_name].to_excel(writer, sheet_name=f"{freq_name}_naive", index=False)
        all_comparison_df[freq_name].to_excel(writer, sheet_name=f"{freq_name}_cmp", index=False)
...
for freq_name in dataframes:
    ...
    fig.suptitle(f"CV Forecasts - {freq_name}", fontsize=12, y=1.01)
    plt.tight_layout()
    plt.savefig(ar_dir / f"forecast_{freq_name.lower()}.pdf", bbox_inches='tight')
    plt.close()
