...
#let robustness-kafka() = {
  let specs = (
    (
      h: 3,
      macro: _parse("/out/holdout/final_holdout/metrics_h3_macro.csv"),
      emb: _parse("/out/holdout/final_emb/metrics_h3_auto.csv"),
      kafka: _parse("/out/holdout/final_kafka/metrics_h3_finbert-kafka.csv"),
      tokens_512: _parse("/out/holdout/final_512/metrics_h3_finbert-512.csv"),
    ),
    (
      h: 6,
      macro: _parse("/out/holdout/final_holdout/metrics_h6_macro.csv"),
      emb: _parse("/out/holdout/final_emb/metrics_h6_auto.csv"),
      kafka: _parse("/out/holdout/final_kafka/metrics_h6_finbert-kafka.csv"),
      tokens_512: _parse("/out/holdout/final_512/metrics_h6_finbert-512.csv"),
    ),
    (
      h: 12,
      macro: _parse("/out/holdout/final_holdout/metrics_h12_macro.csv"),
      emb: _parse("/out/holdout/final_emb/metrics_h12_auto.csv"),
      kafka: _parse("/out/holdout/final_kafka/metrics_h12_finbert-kafka.csv"),
      tokens_512: _parse("/out/holdout/final_512/metrics_h12_finbert-512.csv"),
    ),
  )
  ...
  let mins = specs.map(sp => targets.map(t => model-defs
    .map(mdef => {
      ...
      let src = if srck == "macro" { sp.macro } 
          else if srck == "emb" { sp.emb }
          else if srck == "kafka" { sp.kafka }
          else { sp.tokens_512 }
